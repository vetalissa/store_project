...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'description', 'image', 'quantity', 'price', 'category']
...
class BasketSerializer(serializers.ModelSerializer):
    ...
    def get_total_sum(self, obj):
        return Basket.object.filter(user_id=obj.user.id).total_sum()

    def get_total_quantity(self, obj):
        return Basket.object.filter(user_id=obj.user.id).total_quantity()
